...
export default function FactionList({ status }: { status: 'authenticated' | 'unauthenticated' }) {
    ...
    const loadFactions = async (pageNumber: number) => {
        setLoading(true);
        try {
            const response = await organizationsApi.getFactions(pageNumber);
            const {factions, totalPages} = response.data;
                ...
                <div className="sm:flex sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-2xl/7 font-bold text-gray-900 dark:text-white flex items-center gap-2">
                            ...
                            {t.organizations.galacticFactions}
                        </h2>
                        <p className="mt-1 text-sm text-gray-500">
                            {t.organizations.majorPoliticalMilitary}
                        </p>
                    </div>
                ...
                <ul className="divide-y divide-gray-200 dark:divide-gray-800">
                    {loading ? (
                        ...
                        <li className="px-6 py-12 text-center">
                            ...
                            <h3 className="mt-2 text-sm font-semibold text-gray-900 dark:text-white">{t.organizations.noFactions}</h3>
                            <p className="mt-1 text-sm text-gray-500">
                                {t.organizations.noFactionsCreated}
                            </p>
                        </li>
                        ...
                        const memberCount = faction.totalUniqueMembers || 0;
                                            ...
                                            <div className="flex min-w-0 gap-x-4">
                                                ...
                                                <div className="min-w-0 flex-auto">
                                                    ...
                                                    <p className="mt-1 flex items-center gap-x-2 text-xs/5 text-gray-500 dark:text-gray-400">
                                                        ...
                                                        {t.organizations.leader}: {leader ? `${leader.rank?.name ?? ''} ${leader.character.name}` : t.organizations.vacant}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="flex shrink-0 items-center gap-x-4">
                                                <div className="hidden sm:flex sm:flex-col sm:items-end">
                                                    <p className="text-sm/6 text-gray-900 dark:text-white">
                                                        {memberCount} {memberCount === 1 ? t.organizations.member : t.organizations.members}
                                                        {memberCount !== directMemberCount && (
                                                            <span className="text-xs text-gray-500 ml-1">
                                                                ({directMemberCount} {t.organizations.direct})
                                                            </span>
                                                        )}
                                                    </p>
                                                    <div className="mt-1 flex items-center gap-x-1 5">
                                                        ...
                                                        <p className="text-xs/5 text-gray-500 dark:text-gray-400">
                                                            {isActive ? t.organizations.active : t.organizations.inactive} • {(faction.children ?? []).length} {(faction.children ?? []).length === 1 ? t.organizations.suborganization : t.organizations.suborganizations}
                                                        </p>
                                                    </div>
                                                </div>
