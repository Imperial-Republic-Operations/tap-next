...
import { SerializableNavigationItem } from "@/lib/navigationDB";

interface SidebarNavigationItem extends SerializableNavigationItem {
  isDashboard?: boolean;
}

export default function Sidebar({navigation, session, parentItem}: {
  navigation: SerializableNavigationItem[], 
  session: Session | null,
  parentItem?: SerializableNavigationItem
}) {
    ...
    // Generate navigation items with dashboard link
    const pathname = usePathname();
    
    const sidebarItems: SidebarNavigationItem[] = [];
    
    // Add dashboard link if parentItem exists
    if (parentItem) {
        // For now, always use "Dashboard" - we'll fix the useDashboardTitle logic later
        const dashboardTitle = parentItem.useDashboardTitle ? "Dashboard" : parentItem.title;
        sidebarItems.push({
            ...parentItem,
            id: BigInt(`999${parentItem.id}`), // Generate unique ID
    ...
    sidebarItems.push(...navigation);

    // Function to determine if a link is active
    const isLinkActive = (item: SidebarNavigationItem) => {
        if (item.exact) {
                            ...
                            <li>
                                <ul className="-mx-2 space-y-1">
                                    {sidebarItems
                                        .map((link) => (
                                            <li key={link.id}>
                                                <a href={link.path} className={classNames(isLinkActive(link) ? 'bg-gray-100 text-primary-600 dark:bg-gray-800 dark:text-white' : 'text-gray-700 hover:text-primary-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white', isCollapsed && 'justify-center', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold',)} title={isCollapsed ? link.title : undefined}>
